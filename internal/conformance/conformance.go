...
func ParsePrometheus(r io.Reader) (map[string]Series, error) {
	...
	for scanner.Scan() {
		...
		keep(series, Series{Name: name, Type: types[base], Labels: labels})
	}
...
// keep records a series, preferring whichever observation carries the most
// label keys. A metric's datapoints can legitimately differ -- an exporter
// omits a label it has no value for -- and the widest set is the one a
// dashboard is written against.
func keep(series map[string]Series, s Series) {
	if existing, seen := series[s.Name]; seen && len(existing.Labels) >= len(s.Labels) {
		return
	}
	series[s.Name] = s
}
		...
		for j := 0; j < sms.Len(); j++ {
			...
			for k := 0; k < sm.Metrics().Len(); k++ {
				...
				for l := 0; l < points.Len(); l++ {
					...
					keep(series, Series{Name: metric.Name(), Type: promType, Labels: keys})
				}
			}
		}
	}
...
func Diff(upstream, ours map[string]Series, opts Options) Report {
	...
	for name, up := range upstream {
		...
		if up.Type != mine.Type || !slices.Equal(up.Labels, mine.Labels) {
	...
	slices.SortFunc(report.ShapeMismatch, func(a, b Mismatch) int {
		return strings.Compare(a.Upstream.Name, b.Upstream.Name)
	})
...
func sortSeries(s []Series) {
	slices.SortFunc(s, func(a, b Series) int { return strings.Compare(a.Name, b.Name) })
}
...
func (r Report) Markdown(component, upstream string) string {
	...
	fmt.Fprintf(&b, "Measured by scraping both against the same target, and diffing on\n")
	...
	// Each section ends with a blank line to separate it from the next; the
	// last one would otherwise leave the file ending in one.
